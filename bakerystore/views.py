...
def add_customer(request,customer_id):
    ...
    if request.method == 'POST':
        ...
        if forms.is_valid():
            ...
            return redirect('bakerystore:take-order',o.id)
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form1.is_valid():
            ...
            return redirect('bakerystore:index')
        else:
...
def find_customer(request):
    if request.method=='POST':
        ...
        if Customer.objects.filter(c_phone=number).exists():
            ...
            # Create new order and get its reference directly
            new_order = Order.objects.create(customer=c,order_amount=0)
            return redirect('bakerystore:take-order', new_order.id)
        else:
            new_customer=Customer.objects.create(c_phone=number,c_fname="",c_lname="",c_address="")
            return redirect('bakerystore:add-customer', new_customer.id)
...
def view_order(request):
    o = Order.objects.filter(status='pending').order_by('-order_date')
    return render(request, 'view_order.html', {'o': o})
...
def take_home_delivery(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if forms.is_valid():
            ...
            return redirect('bakerystore:take-order', order_id)
    context = {
        'forms':forms,
        'o':o,
    }
...
def confirm_order(request,order_id):
    ...
    if request.method == 'POST':
        ...
        Order.objects.filter(id=o.id).update(delivery_mode=delivery_mode,feedback=feedback,status='pending')
        ...
        return redirect('bakerystore:printbill',bill.id)
    context = {
        'o':o
    }
...
def add_supplier(request):
    if request.method == 'POST':
        ...
        return redirect('bakerystore:add-supply-order')
...
def add_supply_order(request):
    ...
    if request.method == 'POST':
        ...
        if forms.is_valid():
            ...
            return redirect('bakerystore:view-supply-order')
...
@login_required(login_url='login')
def index(request):
    return redirect('bakerystore:find-customer')  # Using namespaced URL pattern
...
def mark_order_done(request, order_id):
    ...
    return redirect('bakerystore:done-orders')

@login_required(login_url='login')
def mark_order_canceled(request, order_id):
    order = get_object_or_404(Order, id=order_id)
    order.status = 'canceled'
    order.save()
    messages.success(request, 'Order canceled')
    return redirect('bakerystore:canceled-orders')

@login_required(login_url='login')
def done_orders(request):
    orders = Order.objects.filter(status='done').order_by('-order_date')
    return render(request, 'done_orders.html', {'orders': orders})

@login_required(login_url='login')
def canceled_orders(request):
    orders = Order.objects.filter(status='canceled').order_by('-order_date')
    return render(request, 'canceled_orders.html', {'orders': orders})
